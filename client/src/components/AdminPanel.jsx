...
const AdminPanel = forwardRef((props, ref) => {
  ...
  const loadUrls = () => {
    setLoading(true);
    axios
      .get("https://shortener-assignment25.vercel.app/api/admin/list")
      .then((res) => {
  ...
  const deleteUrl = async (id) => {
    try {
      await axios.delete(`https://shortener-assignment25.vercel.app/api/admin/delete/${id}`);
                ...
                {urls.map((url) => (
                  <tr key={url._id} className="hover:bg-gray-200 transition">
                    ...
                    <td className="p-3 border">
                      <a
                        href={`https://shortener-assignment25.vercel.app/api/${url.short_code}`}
                        target="_blank"
